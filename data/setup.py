...
BASE_VOICES = set(
    "aleksandr anatol anna azamat bdl clb elena irina Leticia-F123 natalia natia nazgul slt spomenka "
    "talgat".split(" ")
)
...
class RHVoiceBuild(build):
            ...
            if os.path.exists(dst__):
                for x in range(1, 9999):
                    ...
                    if not os.path.exists(dst__):
                        self.warn(
                            "File {} already exists in {}, save is as {}!".format(
                                filename, dst_path, filename_2
                            )
                        )
                        break
                else:
        ...
        for voice in voices:
            ...
            for target in os.listdir(src_voice):
                if target.lower().startswith(("license", "readme")):
                    self._copy_file(
                        target, src_voice, os.path.join(dst, "voices", voice)
                    )
    ...
    def run(self):
        ...
        targets = {
            "voices": (
                os.path.join(src_path, DATA_DIR, "voices"),
                os.path.join(dst_path, DATA_DIR, "voices"),
            ),
            "languages": (
                os.path.join(src_path, DATA_DIR, "languages"),
                os.path.join(dst_path, DATA_DIR, "languages"),
            ),
            "licenses": (
                os.path.join(src_path, LICENSES_DIR),
                os.path.join(dst_path, LICENSES_DIR),
            ),
        }
...
setup(
    ...
    package_data={
        PACKAGE_PATH: [os.path.join(x, "*") for x in (DATA_DIR, LICENSES_DIR)]
    },
